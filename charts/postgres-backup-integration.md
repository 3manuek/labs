...
```mermaid
graph TB
    subgraph "pgbackrest.conf Structure"
        GLOBAL["[global]<br/>repo1-type=s3<br/>repo1-s3-bucket=pg-backups<br/>repo1-s3-region=us-east-1<br/>repo1-retention-full=4<br/>repo1-retention-diff=4<br/>process-max=4"]

        STANZA["[main]<br/>pg1-path=/var/lib/postgresql/data<br/>pg1-port=5432<br/>pg1-host=patroni-primary<br/>pg2-host=patroni-replica1<br/>pg3-host=patroni-replica2"]

        S3_AUTH["S3 Authentication<br/>repo1-s3-key=ACCESS_KEY<br/>repo1-s3-key-secret=SECRET<br/>OR<br/>IAM Role (preferred)"]

        ENCRYPT["Encryption<br/>repo1-cipher-type=aes-256-cbc<br/>repo1-cipher-pass=secret"]

        COMPRESS["Compression<br/>compress-type=lz4<br/>compress-level=3"]
    end
    ...
    style GLOBAL fill:#e67e22,stroke:#ca6f1e,stroke-width:3px
    style STANZA fill:#3498db,stroke:#2980b9,stroke-width:2px
    style S3_AUTH fill:#e74c3c,stroke:#c0392b,stroke-width:2px
...
1. **Patroni Bootstrap**
   - Can bootstrap new replicas from pgBackRest backups
   - Faster than pg_basebackup for large databases
   - Automatic standby configuration

2. **WAL Archiving**
   - Primary archives WAL to S3 via pgBackRest
   - Replicas can fetch WAL from S3 if streaming lag occurs
   - Continuous protection without interruption

3. **Automatic Failover**
   - New primary immediately starts WAL archiving
   - No backup gap during failover
   - Old primary can rejoin using WAL from S3

4. **Backup from Replica**
   - Reduces load on primary
   - Patroni ensures replica is in sync
   - Automatic selection of best backup source

5. **Monitoring Integration**
   - Patroni health checks include backup status
   - Backup metrics exposed to Prometheus
   - Automated alerts for backup issues
