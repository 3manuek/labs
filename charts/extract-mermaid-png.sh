    ...
    while IFS= read -r line; do
        ...
        if [ $in_mermaid -eq 1 ]; then
            if echo "$line" | grep -q "^\`\`\`"; then
                ...
                png_file="$DOWNLOADS_DIR/${base_name}-$(printf "%02d" $diagram_num).png"
                echo "  Converting diagram $diagram_num -> $png_file"
                if mmdc -i "$mmd_file" -o "$png_file" -b transparent 2>/dev/null; then
                    count=$((count + 1))
                else
